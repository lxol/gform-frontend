...
sealed trait ExpressionResult extends Product with Serializable {
  ...
  def +(er: ExpressionResult)(implicit messages: Messages): ExpressionResult = er match {
    ...
    case t: ListResult => fold[ExpressionResult](identity)(identity)(identity)(e => ListResult(t.list.map(_ + e)))(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a + b }))
    // format: on
  }
  ...
  def -(er: ExpressionResult): ExpressionResult = er match {
    ...
    case t: ListResult => fold[ExpressionResult](identity)(identity)(identity)(e => ListResult(t.list.map(e - _)))(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a - b }))
    // format: on
  }
